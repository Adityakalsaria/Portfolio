...
/** The unmodified photo the reveal melts back to. */
const REAL_URL = "/portrait.webp";
...
/**
 * One AI-restyled version of the same photo per entry. Any that is missing
 * falls back to the real photo, so the effect runs before they are supplied.
 */
const STYLES: Style[] = [
  { url: "/lego/lego.webp", bg: [253, 246, 216], glow: [255, 222, 80], zoom: 1.12 },
  { url: "/lego/minecraft.webp", bg: [231, 244, 224], glow: [126, 211, 90] },
  { url: "/lego/roblox.webp", bg: [240, 238, 240], glow: [226, 78, 78] },
];
...
export class LegoReveal {
  ...
  private loadTexture(url: string, done: (t: WebGLTexture, ar: number) => void) {
    ...
    // A style that has not been supplied yet falls back to the real photo, so
    // the card shows a portrait rather than a grey placeholder.
    img.onerror = () => {
      if (url !== REAL_URL && !this.destroyed) this.loadTexture(REAL_URL, done);
    };
    img.src = url;
  }
  ...
  private frame(now: number) {
    ...
    const FX = 0.5;
    const FY = 0.66;
    const ax = FX + 0.16 * Math.sin(T * 1.15) + 0.09 * Math.sin(T * 2.6 + 1.0) + 0.05 * Math.cos(T * 4.1 + 0.4);
    const ay = FY + 0.13 * Math.cos(T * 0.95 + 2.1) + 0.07 * Math.sin(T * 2.2 + 0.5) + 0.04 * Math.cos(T * 3.7 + 1.7);
